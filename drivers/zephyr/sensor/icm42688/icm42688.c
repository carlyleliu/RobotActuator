...
static int icm42688_set_accel_fs(const struct device *dev, uint16_t fs)
{
	...
	return icm42688_spi_update_register(&cfg->spi, REG_ACCEL_CONFIG0,
					    (uint8_t)MASK_ACCEL_UI_FS_SEL, temp);
}

static int icm42688_set_gyro_fs(const struct device *dev, uint16_t fs)
{
	const struct icm42688_config *cfg = dev->config;
	struct icm42688_data *data = dev->data;
	uint8_t temp;
	...
	data->gyro_sensitivity_x10 = icm42688_gyro_sensitivity_x10[temp];

	return icm42688_spi_update_register(&cfg->spi, REG_GYRO_CONFIG0,
					    (uint8_t)MASK_GYRO_UI_FS_SEL, temp);
}

static int icm42688_set_accel_odr(const struct device *dev, uint16_t rate)
{
	const struct icm42688_config *cfg = dev->config;
	uint8_t temp;
    ...
    if(rate > 16000) {
        temp = BIT_ACCEL_ODR_32000;
    } else if (rate > 8000) {
		temp = BIT_ACCEL_ODR_16000;
	} else if (rate > 4000) {
	...
	return icm42688_spi_update_register(&cfg->spi, REG_ACCEL_CONFIG0, (uint8_t)MASK_ACCEL_ODR, temp);
}

static int icm42688_set_gyro_odr(const struct device *dev, uint16_t rate)
{
	const struct icm42688_config *cfg = dev->config;
	uint8_t temp;
    ...
    if (rate > 16000) {
        temp = BIT_GYRO_ODR_32000;
    } else if (rate > 8000) {
		temp = BIT_GYRO_ODR_16000;
	} else if (rate > 4000) {
	...
	return icm42688_spi_update_register(&cfg->spi, REG_GYRO_CONFIG0, (uint8_t)MASK_GYRO_ODR,
					    temp);
}

static int icm42688_sensor_init(const struct device *dev)
{
	int res;
	uint8_t value;
	const struct icm42688_config *cfg = dev->config;
	...
	/* force Reset hardware configuration (so that next read is correct) */
	res = icm42688_spi_single_write(&cfg->spi, REG_DEVICE_CONFIG, BIT_SPI_RESET);
	...
	res = icm42688_spi_read(&cfg->spi, REG_INT_STATUS, &value, 1);
	...
	res = icm42688_spi_read(&cfg->spi, REG_WHO_AM_I, &value, 1);
...
static int icm42688_turn_on_sensor(const struct device *dev)
{
	struct icm42688_data *data = dev->data;
	const struct icm42688_config *cfg = dev->config;
	uint8_t value;
	...
	res = icm42688_spi_update_register(&cfg->spi, REG_PWR_MGMT0,
...
{
	int res = 0;
	const struct icm42688_data *data = dev->data;

	icm42688_lock(dev);

	switch (chan) {
	case SENSOR_CHAN_ACCEL_XYZ:
		icm42688_convert_accel(&val[0], data->accel_x, data->accel_sensitivity_shift);
		icm42688_convert_accel(&val[1], data->accel_y, data->accel_sensitivity_shift);
		icm42688_convert_accel(&val[2], data->accel_z, data->accel_sensitivity_shift);
		break;
	case SENSOR_CHAN_ACCEL_X:
		icm42688_convert_accel(val, data->accel_x, data->accel_sensitivity_shift);
		break;
	case SENSOR_CHAN_ACCEL_Y:
		icm42688_convert_accel(val, data->accel_y, data->accel_sensitivity_shift);
		break;
	case SENSOR_CHAN_ACCEL_Z:
		icm42688_convert_accel(val, data->accel_z, data->accel_sensitivity_shift);
		break;
	case SENSOR_CHAN_GYRO_XYZ:
		icm42688_convert_gyro(&val[0], data->gyro_x, data->gyro_sensitivity_x10);
		icm42688_convert_gyro(&val[1], data->gyro_y, data->gyro_sensitivity_x10);
		icm42688_convert_gyro(&val[2], data->gyro_z, data->gyro_sensitivity_x10);
		break;
	case SENSOR_CHAN_GYRO_X:
		icm42688_convert_gyro(val, data->gyro_x, data->gyro_sensitivity_x10);
		break;
	case SENSOR_CHAN_GYRO_Y:
		icm42688_convert_gyro(val, data->gyro_y, data->gyro_sensitivity_x10);
		break;
	case SENSOR_CHAN_GYRO_Z:
		icm42688_convert_gyro(val, data->gyro_z, data->gyro_sensitivity_x10);
		break;
	case SENSOR_CHAN_DIE_TEMP:
		icm42688_convert_temp(val, data->temp);
		break;
	default:
		res = -ENOTSUP;
		break;
...
static int icm42688_sample_fetch_accel(const struct device *dev)
{
	const struct icm42688_config *cfg = dev->config;
	struct icm42688_data *data = dev->data;
	uint8_t buffer[ACCEL_DATA_SIZE];

	int res = icm42688_spi_read(&cfg->spi, REG_ACCEL_DATA_X1, buffer, ACCEL_DATA_SIZE);
...
static int icm42688_sample_fetch_gyro(const struct device *dev)
{
	const struct icm42688_config *cfg = dev->config;
	struct icm42688_data *data = dev->data;
	uint8_t buffer[GYRO_DATA_SIZE];

	int res = icm42688_spi_read(&cfg->spi, REG_GYRO_DATA_X1, buffer, GYRO_DATA_SIZE);
...
static int icm42688_sample_fetch_temp(const struct device *dev)
{
	const struct icm42688_config *cfg = dev->config;
	struct icm42688_data *data = dev->data;
	uint8_t buffer[TEMP_DATA_SIZE];

	int res = icm42688_spi_read(&cfg->spi, REG_TEMP_DATA1, buffer, TEMP_DATA_SIZE);
...
static int icm42688_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_ALL:
		res |= icm42688_sample_fetch_accel(dev);
		res |= icm42688_sample_fetch_gyro(dev);
		res |= icm42688_sample_fetch_temp(dev);
		break;
	case SENSOR_CHAN_ACCEL_XYZ:
	case SENSOR_CHAN_ACCEL_X:
	case SENSOR_CHAN_ACCEL_Y:
...
static int icm42688_init(const struct device *dev)
{
	...
	int res = icm42688_turn_on_sensor(dev);
...
#define ICM42688_INIT(inst)                                                                        \
	static struct icm42688_data icm42688_driver_##inst = {                                     \
	...
	static const struct icm42688_config icm42688_cfg_##inst = {                                \
		.spi = SPI_DT_SPEC_INST_GET(inst, ICM42688_SPI_CFG, 1U),                           \
