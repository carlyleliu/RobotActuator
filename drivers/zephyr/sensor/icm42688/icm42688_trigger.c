				   ...
				   uint32_t pins)
{
	...
	struct icm42688_data *data = CONTAINER_OF(cb, struct icm42688_data, gpio_cb);
...
int icm42688_trigger_init(const struct device *dev)
{
	...
	gpio_init_callback(&data->gpio_cb, icm42688_gpio_callback, BIT(cfg->gpio_int.pin));
...
int icm42688_trigger_enable_interrupt(const struct device *dev)
{
	...
	return icm42688_spi_single_write(&cfg->spi, REG_INT_SOURCE0, BIT_INT_DRDY_INT1_EN);
}
