...
{
    float theta = electrical_angle_;
    if (i_alpha_beta_measured_.has_value()) {
        auto [Ialpha, Ibeta] = *i_alpha_beta_measured_;
        float cos_theta = arm_cos_f32(theta);
        float sin_theta = arm_sin_f32(theta);
        i_dq_measured_ = {
            Ialpha * cos_theta + Ibeta * sin_theta,
            -Ialpha * sin_theta + Ibeta * cos_theta,
        };
    }
}
 ...
 */
void FieldOrientedController::FocRevPark(void)
{
    float theta = electrical_angle_;
    if (v_dq_target_.has_value()) {
        auto [Vd, Vq] = *v_dq_target_;
        float cos_theta = arm_cos_f32(theta);
        float sin_theta = arm_sin_f32(theta);
        v_alpha_beta_target_ = {
            Vd * cos_theta - Vq * sin_theta,
